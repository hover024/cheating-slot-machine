...
describe('Utils Functions', () => {
  describe('getRandomFruit', () => {
    it('should return fruits with approximately uniform distribution', () => {
      ...
      for (const fruit in results) {
        expect(results[fruit as Fruit]).toBeGreaterThanOrEqual(expectedCount - tolerance);
        expect(results[fruit as Fruit]).toBeLessThanOrEqual(expectedCount + tolerance);
      }
    });
  });
  ...
  describe('computeRollResult', () => {
    ...
    it('should return the correct cost if all elements in result array are the same', () => {
      ...
      const watermelonResult = computeRollResult(["watermelon", "watermelon", "watermelon"]);
